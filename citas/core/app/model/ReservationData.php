...
class ReservationData {
	...
	public function add(){
		$sql = "insert into reservacion (title,note,medic_id,date_at,time_at,pacient_id,user_id,price,status_id,sick,symtoms,medicaments,created_at) ";
		$sql .= "value (\"$this->title\",\"$this->note\",\"$this->medic_id\",\"$this->date_at\",\"$this->time_at\",$this->pacient_id,$this->user_id,\"$this->price\",$this->status_id,\"$this->sick\",\"$this->symtoms\",\"$this->medicaments\",$this->created_at)";
...
// partiendo de que ya tenemos creado un objecto ReservationData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set title=\"$this->title\",pacient_id=\"$this->pacient_id\",medic_id=\"$this->medic_id\",date_at=\"$this->date_at\",time_at=\"$this->time_at\",note=\"$this->note\",sick=\"$this->sick\",symtoms=\"$this->symtoms\",medicaments=\"$this->medicaments\",status_id=\"$this->status_id\",price=\"$this->price\" where id=$this->id";
		Executor::doit($sql);
	}
